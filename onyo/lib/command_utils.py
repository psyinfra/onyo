...
def whosyourdaddy(inventory: Inventory,
    ...
    for item in items:
        if item["onyo.path.absolute"] == path:
            ...
            continue
        parent_candidates = [i for i in items if i["onyo.path.relative"] == item["onyo.path.parent"]]
        assert len(parent_candidates) == 1
        if parent_candidates[0]["onyo.is.asset"]:
            parent_id = parent_candidates[0]["onyo.documentid"]
            item["onyo.path.parent"] = f"<?onyo.documentid={parent_id}>"
        else:
            item["onyo.path.parent"] = str((inventory.root / item["onyo.path.parent"]).relative_to(base))
    ...
    for item in items:
        for key in list(PSEUDO_KEYS.keys()):
            if key in ["onyo.is.asset", "onyo.is.directory", "onyo.path.parent"] or \
                    (key == "onyo.path.name" and not item["onyo.is.asset"]):
                continue
            del item[key]
        # TODO: needs actual namespace handling to be nicer (as in: `item.remove_namespace("onyo.was")`):
        del item["onyo.was"]
