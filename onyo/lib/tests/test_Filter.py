...
def test_filter_equal(filt: str) -> None:
...
def test_filter_greater_than(filt: str) -> None:
    """Test whether ``Filter`` object are set up, including post-initialization."""
    ...
    assert f.operand == '>'
    assert f.value == filt.split('>', 1)[1]
    assert not f.match(read_asset('laptop_make_model.1'))

    match filt.split('>', 1)[1]:
        case 'laptop':
            assert f.match(read_asset('monitor_make_model.2'))
            assert not f.match(read_asset('headphones_make_model.3'))
            assert f.match(read_asset('wheelchair_make_model.4'))
            assert f.match(read_asset('wheelchair_make_model.5'))
        case 'value':
...
@pytest.mark.parametrize('filt', ['type>=laptop', 'key>=value', 'foo>=<unset>', 'serial>=2'])
def test_filter_greater_than_or_equal(filt: str) -> None:
    """Test whether ``Filter`` object are set up, including post-initialization."""
...
@pytest.mark.parametrize('filt', ['type<laptop', 'key<value', 'foo<<unset>', 'serial<2'])
def test_filter_less_than(filt: str) -> None:
    """Test whether ``Filter`` object are set up, including post-initialization."""
...
@pytest.mark.parametrize('filt', ['type<=laptop', 'key<=value', 'foo<=<unset>', 'serial<=2'])
def test_filter_less_than_or_equal(filt: str) -> None:
    """Test whether ``Filter`` object are set up, including post-initialization."""
